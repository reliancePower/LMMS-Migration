(function() {
	...
	angular.module('regCalApp.viewnotice').controller('ViewNoticeController',
			ViewNoticeController);

	ViewNoticeController.$inject = [ '$state', 'viewCaseService',
			...
			'ViewNoticeService', 'AddNoticeService', 'ManageNoticesDocsService', 'navbarService' ];

	/* @ngInject */
	function ViewNoticeController($state, NoticeService, localStorageService,
		...
		function viewNoticeItem(item) {
			...
			ManageNoticesDocsService.fetchLmmsDocuments(json).then(
					function(result) {
						...
						vm.paperArray = result.data.docArray;
						vm.company = item.companyAlias;
						vm.business = item.businessName;
						vm.vertical = item.verticalName;
						vm.caseType = item.caseTypeName;
						vm.caseTypeID = item.caseType;
						vm.panNo = item.panNo;
						vm.assessYear = item.assessYear;
						vm.noticeType = item.noticeType;
						vm.section = item.noticeSection;
						vm.sectionDesc = item.sectionDesc;
						vm.dateOnNotice = $filter('date')(item.dateOnNotice,
								'dd/MM/yyyy');
						vm.dateOnWhichNoticeReceived = $filter('date')(
								item.dateOnWhichNoticeReceived, 'dd/MM/yyyy');
						vm.dateOnWhichNoticeRequiredToReply = $filter('date')(
								item.dateOnWhichNoticeRequiredToReply,
								'dd/MM/yyyy');
						vm.dateOfReply = $filter('date')(item.dateOfReply,
								'dd/MM/yyyy');
						vm.state = item.stateName;
						vm.issuedByName = item.issuedByName;
						vm.issuedByDesig = item.issuedByDesig;
						vm.issuedByOrganization = item.issuedByOrganization;
						vm.addressedTo = item.addressedTo;
						vm.natureOfParty = item.natureOfParty;
						vm.briefFacts = item.briefFacts;
						vm.stakeInvolment = item.stakeInvolment;
						vm.priority = item.priorityName;
						vm.remarks = item.remarks;
						vm.status = item.statusName;
						vm.enteredBy = item.enteredBy;
		...
		function fetchAllNotices() {
			ViewNoticeService.fetchAllNotices().then(function(result) {
				if(result.data.status == 'F'){
					alert(result.data.msg);
					return false;
				}
		...
		function search(form) {
			...
			if (vm.noticeStatusInfo.length == $('#noticeStatus').val().length)
				noticeStatus = '0';
			else
				noticeStatus = $('#noticeStatus').val().toString();
			...
			var jsonReq = {
				...
				"state" : state,
				"dateOnNoticeSDate" : stDate,
				"dateOnNoticeEDate" : endDate,
			...
			ViewNoticeService.searchNotices(jsonReq).then(function(result) {
				if(result.data.status == 'F'){
					alert(result.data.msg);
					return false;
				}
