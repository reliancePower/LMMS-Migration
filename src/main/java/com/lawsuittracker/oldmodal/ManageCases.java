...
public class ManageCases {
	...
	public String addUpdateNewCase(String input) {
		String response = "";
		ResultSet rs = null;
		Connection con = null;
				...
				if ((rs.getInt(1) == 1) && (finImpArray.length() > 0)
					...
					for (int i = 0; i < finImpArray.length(); i++) {
						try {
							...
							stmt.setLong(4, Long.valueOf(finImpArray.getJSONObject(i).getLong("penalty")).longValue());
							stmt.setLong(5, Long.valueOf(finImpArray.getJSONObject(i).getLong("interest")).longValue());
							stmt.setLong(6, Long.valueOf(finImpArray.getJSONObject(i).getLong("total")).longValue());
	...
	public String fetchAllCases(String input, String sessionID, String sessUser) {
		...
		String endDateLast = (obj.has("endDateLast")) ? obj.getString("endDateLast") : "";
		String nextDateNA = (obj.has("nextDateNA")) ? obj.getString("nextDateNA") : "";
		String judgeResPronCheck = (obj.has("judgeResPronCheck")) ? obj.getString("judgeResPronCheck") : "";
	...
	public String fetchCaseDetails(String input, String sessionID, String sessUser) {
		...
		try {
			...
			String query = "SELECT id,company_name,business_name,vertical_name,case_no "
					...
					+ " FROM vw_case_view_master where id = ? and id in (select id from vw_case_user_master where user_id = ? and id = ?)";
			stmt = con.prepareStatement(query);
			...
			if (rs.next()) {
				...
				String sql3 = "SELECT id, mongo_db_id, file_name, file_type, file_desc, document_type, document_type_name, source, created_by, user_name, CONVERT(varchar, timestamp, 100), CONVERT(varchar, document_date, 103), confidential  from vw_document_details where case_id  = ? order by cast(timestamp as datetime) desc";
