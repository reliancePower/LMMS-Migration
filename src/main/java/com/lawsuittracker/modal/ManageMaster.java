...
public class ManageMaster {
	...
	public String fetchRefNo(String input, String sessionID, String sessUser) {
		String response = "";
		Connection con = null;
		...
		if (sessionID == null) {
			...
			try {
				...
				if ("business".equalsIgnoreCase(type) && (!("".equals(id)))) {
					...
					if ("All".equalsIgnoreCase(id)) {
						query = "SELECT distinct id, alias, vertical_name from vw_user_vertical where user_id = '"
								+ userID + "' order by vertical_name";
						// System.out.println(query);
						stmt = con.prepareStatement(query);
					} else {
	...
	public String pvForumUpdateLog(String input, String sessionID, String sessUser) {
		String response = "";
		Connection con = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
			...
			if ("fetchMaster".equalsIgnoreCase(type)) {
						...
						if (rs1.next()) {
							if (rs1.getInt(1) > 0) {
								...
								String template = "<html><body style=\"font-family:Georgia;font-size:14;\"><p> Dear Admin, </p>	<p> A new forum addition request is pending with you for approval. <br><br>Kindly take the appropriate action. PFB - the details <p> <table border = \"1\" style=\"font-family:Georgia;font-size:12;\">"
										+ "<tr><td><b>Forum Category</b></td><td>"
										+ StringUtils.capitaliseAllWords(category)
										+ "</td></tr><tr><td><b>Name</b></td><td>" + name + ", " + state
				...
				if (rs.next()) {
					...
					if (rs.getInt(1) > 0) {
						...
						String template = "<html><body style=\"font-family:Georgia;font-size:14;\"><p> Dear Admin, </p>	<p> A new forum addition request is pending with you for approval. <br><br>Kindly take the appropriate action. PFB - the details <p> <table border = \"1\" style=\"font-family:Georgia;font-size:12;\">"
								+ "<tr><td><b>Forum Category</b></td><td>" + StringUtils.capitaliseAllWords(category)
								+ "</td></tr><tr><td><b>Name</b></td><td>" + name + ", " + state
								+ "</td></tr></table></p><p>Please click <a href= \"https://mportal.reliancepower.co.in/LMMS/\">here</a> to approve the forum</p><p><strong>Regards,</strong></p><p>LMMS admin Team</p><br><br><p><b>Note : </b>This is system generated mail. Hence do not reply</p></body></html>";
						System.out.println(template);
						sm.sendAMail(LawSuitTrackerConstants.adminMail, sub, template, "",
				...
				while (rs.next()) {
					...
					jobj.put("userName", rs.getString(6));
					jobj.put("mobileNo", rs.getString(7));
					resultArray.put(jobj);
				}
				outJson.put("resultArray", resultArray);
	...
	public String fetchAllMasters(String input, String sessionID, String sessUser) {
		...
		if (sessionID == null) {
			j.put(LawSuitTrackerConstants.status, "S");
			j.put("msg", "Your session has expired. Please logout and login again...");
		} else if ("".equals(sessID) || (!sessionID.equals(sessID))) {
			j.put(LawSuitTrackerConstants.status, "S");
			j.put("msg", "Your session is invalid. Please try again later!");
