	...
	public String searchAlert(String input, String sessionID, String sessUser) {
		...
		if (sessionID == null) {
			...
			try {
				...
				String query = "declare @comp varchar(100) = '';declare @busi varchar(MAX) = '';declare @vert varchar(MAX) = '';declare @caseTy varchar(MAX) = '';select @busi = @busi + isnull(cast(id as varchar), '') + ',' from vw_user_business where user_id = '"
						+ userID + "'; "
						+ "select @vert = @vert + isnull(cast(id as varchar), '') + ',' from vw_user_vertical where user_id = '"
						+ userID
						+ "';select @comp = @comp + isnull(cast(id as varchar), '') + ',' from vw_user_company where user_id = '"
						+ userID
						+ "';select @caseTy = @caseTy + isnull(cast(case_id as varchar), '') + ',' from vw_user_case_type where payroll_no = '"
						+ userID
				...
				query = query
						+ "  order by pv_notification_type desc,timestamp desc,company_alias,forum option(maxrecursion 0);";
		...
		if (sessionID == null) {
			...
			try {
				...
				cstmt = con.prepareCall(
						"{call " + LawSuitTrackerConstants.alertNotificationProc + "(?,?,?,?,?,?,?,?,?,?)}");
